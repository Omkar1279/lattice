...
def handle_stop(payload: str) -> str:
    '''Final session hook: prunes old auto-captures and writes a session summary.'''
    ...
    try:
        ...
        cursor = vault.db.execute('DELETE FROM chunks WHERE source = "auto_capture" AND last_seen_at < ?', (cutoff,))
        if cursor.rowcount > 0:
            vault.db.execute("INSERT INTO chunks_fts(chunks_fts) VALUES('delete-all')")
            vault.db.execute("INSERT INTO chunks_fts(chunks_fts) VALUES('rebuild')")
